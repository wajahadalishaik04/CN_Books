...
const VerifyScreen = () => {
  ...
  const [isVerifyEnabled, setIsVerifyEnabled] = useState(false); // State to enable/disable the Verify button
  ...
  // Automatically generate OTP on component mount or when signup form is submitted
  useEffect(() => {
    handleResendOTP();
  }, []);
  ...
  // Enable Verify button when OTP is filled
  useEffect(() => {
    if (otp.length === 6) {
      ...
      >
        <Box flex={1} p={6} mt={1}>
          <Box flex={1} width={"330px"}>
            ...
            {/* Enable Verify button only when OTP is filled */}
            <StyledButton
              onClick={handleSubmit}
              variant="contained"
              fullWidth
              sx={{ backgroundColor: "#5BC4FA", mt: 1 }}
              disabled={!isVerifyEnabled} // Disable if OTP isn't filled
            >
              Verify
            </StyledButton>
          </Box>
