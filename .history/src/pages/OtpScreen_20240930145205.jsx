...
const OtpScreen = () => {
  ...
  const generateOtp = () => {
    ...
    setOtp(randomOtp); // Also set the OTP in the input field
    return randomOtp;
  };
  ...
  const sendOtp = () => {
    ...
    handleSnackbarOpen(`OTP sent: ${otp}`); // Show OTP in snackbar
  };
      ...
      >
        ...
        <Box flex={1} p={4.5} mt={1} component={"form"}>
          <Box flex={1} sx={{ width: "330px" }}>
            ...
            {/* OTP Input with End Adornment (OTP) */}
            <TextField
              size="small"
              type="text"
              ...
              InputProps={{
                endAdornment: sentOtp && (
                  <InputAdornment position="end">
                    <IconButton
                      onClick={() => navigator.clipboard.writeText(sentOtp)}
                      edge="end"
                      aria-label="copy otp"
                    >
                      <ContentCopyIcon />
